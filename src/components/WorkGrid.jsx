  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      {PROJECTS.map((p, idx) => (
        ...
        >
          <div className="relative h-44 overflow-hidden rounded-2xl border border-cyan-200/35 bg-cyan-400/15">
            <img
              src={p.image}
              alt={p.title}
              loading="lazy"
          ...
          <div className="mt-5">
            ...
            <p className="mt-2 text-slate-100/85">{p.description}</p>

            <div className="mt-4 rounded-2xl border border-cyan-200/35 bg-cyan-400/15 p-4">
              <div className="text-xs font-bold uppercase tracking-wide text-cyan-200/80">Challenge</div>
              <p className="mt-2 text-sm text-slate-100/85">Need for clearer data visibility and faster team execution across the product lifecycle.</p>

              <div className="mt-4 text-xs font-bold uppercase tracking-wide text-cyan-200/80">Solution</div>
              <div className="mt-2 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span key={t} className="text-xs rounded-full px-3 py-1 bg-cyan-400/15 border border-cyan-200/35 text-slate-100/85">{t}</span>
                ))}
              </div>
              ...
              <ul className="mt-2 space-y-2">
                {outcomes.map((o) => (
                  <li key={o} className="flex items-start gap-2 text-sm text-slate-100/85">
            ...
            <a href={p.link || "#contact"} className="mt-6 inline-flex items-center gap-2 text-sm text-slate-100/85 hover:text-white transition">
