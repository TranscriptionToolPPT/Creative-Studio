    ...
    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-5">
      {impacts.map((item, idx) => (
        <motion.div
          ...
          className="card"
        >
          <div className="text-3xl font-black bg-gradient-to-r from-violet-200 to-fuchsia-200 bg-clip-text text-transparent">
            <Counter target={item.value} suffix={item.suffix} />
          </div>
          <div className="mt-3 text-sm text-violet-100/80 leading-relaxed">{item.label}</div>
        </motion.div>
      ))}
    </div>
  )
